...
class ManagerPrivateMessages2 extends Manager {
    ...
    protected function initialize_ManagerPrivateMessages2() {
    ...
    public function sendReply($f_message, $f_conversation, $f_reader=false) {
        ...
        if ($obj_messages->insert_return($new_message)) {
            ...
            if ($recipient['member_settings_notification_new_message']) {
                $new_email['message_id'] = $new_message['message_conversation'];
                $new_email['mainurl'] = $this->pageData['mainurl'];
                $new_email['email'] = $recipient['member_email'];
                $new_email['username'] = $recipient['member_username'];
                $new_email['sender'] = $sender['member_username'];
...
//TODO Mark Conversation as read.
    ...
    public function toggleConversationFolder($f_conversation, $f_folder, $f_reader=false) {
        ...
        $conversation = $obj_conversations->load($f_conversation, $this->myCurrentMember);
...
// Check conversations.
    ...
    public function loadMemberByUnique($f_member_unique) {
        ...
        if (is_numeric($f_member_unique)) {
            $where['member'] = $f_member_unique;
        } else {
            $where['member_unique'] = $this->encodeString($f_member_unique);
        }
    ...
    public function checkMessageFilters($f_recipient, $f_sender=false) {
        ...
        /* $obj_whitelists = $this->loadClass('PRIVATE_MESSAGES_WHITELIST_CLASS_NAME');
          ...
          //WHITELIST OPTIONS
          //FAVORITE WHITELIST
          if ($recipient['member_settings_whitelist_favorites'] && $sender['is_favorite']) {
          return PRIVATE_MESSAGE_RETURN_NO_FILTER;
          }
          //FRIEND WHITELIST
          if ($recipient['member_settings_whitelist_friends'] && $sender['are_friends']) {
          return PRIVATE_MESSAGE_RETURN_NO_FILTER;
          }
          if ($recipient['member_settings_whitelist_messaged'] && $sender['have_contacted']) {
          return PRIVATE_MESSAGE_RETURN_NO_FILTER;
          }
          if ($recipient['member_settings_whitelist_premium'] && $sender['member_premium']) {
          return PRIVATE_MESSAGE_RETURN_NO_FILTER;
          }
          if ($recipient['member_settings_whitelist_distance'] && $sender['distance'] < $recipient['member_settings_whitelist_distance']) {
          return PRIVATE_MESSAGE_RETURN_NO_FILTER;
          }
          ...
          //PHOTOS FILTER
          if ($recipient['member_settings_message_filter_photos'] && $sender['member_number_photos'] < $recipient['member_settings_message_filter_photos']) {
          return PRIVATE_MESSAGE_RETURN_FILTER;
          }
          if ($recipient['member_settings_message_filter_distance'] && $sender['distance'] > $recipient['member_settings_message_filter_distance']) {
          return PRIVATE_MESSAGE_RETURN_FILTER;
          }
          if ($recipient['member_settings_message_filter_joined'] && $sender['member_days_as_member'] > $recipient['member_settings_message_filter_joined']) {
          return PRIVATE_MESSAGE_RETURN_FILTER;
          }
          if ($recipient['member_settings_message_filter_min_age'] && $sender['member_age'] < $recipient['member_settings_message_filter_min_age']) {
          return PRIVATE_MESSAGE_RETURN_FILTER;
          }
          if ($recipient['member_settings_message_filter_max_age'] && $sender['member_age'] > $recipient['member_settings_message_filter_max_age']) {
          return PRIVATE_MESSAGE_RETURN_FILTER;
          }
          if ($recipient['member_settings_message_filter_must_be_single'] && $sender['member_relationship_status'] != 17) {
