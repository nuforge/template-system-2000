...
class email {
    ...
    function mimeDecode($mailtext) {
        ...
        if (!empty($this->recipients)) {
            if (is_array($this->recipients)) {
                $parts[1]['To'] = implode(', ', $this->recipients);
            } else {
                $parts[1]['To'] = $this->recipients;
            }
        }
        return $parts;
    }
    ...
    function sendEmailTemplate($templateFile, $args=array(), $hdrs=false, $f_method='sendmail') {
        ...
        if ($hdrs) {
            $headers = $this->formatHeaders($hdrs);
        } else {
            $parts = $this->mimeDecode($body);
            if (empty($this->recipients)) {
                $this->recipients = $parts[0];
            }
        ...
        $mail = Mail::factory($f_method, $mailOptions);
        return PEAR::isError($mail) ? $mail : $mail->send($this->recipients, $headers, $body, null, '-fadmin@webcomicz.com');
    }
