...
class DB_Object {
    ...
    protected $metaData;
    protected $meta_joined;
    protected $defaultQuery;
    protected $SELECT_STATMENTS;
    ...
    public function __construct($fobj_db_connection=false, $f_table=false, $fa_connection_info=false) {
        ...
        DEFINE('SELECT_QUERIES_TITLE', 'queries');
        DEFINE('SELECT_BOOLEANS_TITLE', 'booleans');
        DEFINE('QUERY_GROUP_BY_TITLE', 'group_by');
        DEFINE('QUERY_HAVING_TITLE', 'group_by');
        DEFINE('DISTINCT_ON_TITLE', 'distinct_on');
        DEFINE('SELECT_SIMPLE_LIST_QUERY', 'simple_list');
        DEFINE('SELECT_SELECT_LIST_QUERY', 'select_list');

        $this->defaultQuery = get_class($this) . '_default';

        $this->addSelectStatement('*');
        $this->addSelectStatement($this->primarykey, false, SELECT_SIMPLE_LIST_QUERY);
    ...
    public function setMeta($fobj_database_connection=false, $f_table=false) {
        ...
        $this->metaData[$f_table] = $this->meta;
    ...
    public function joinTable($f_class_name, $f_on=false, $f_join_type=false, $f_as=false, $f_query_title = false) {
        $f_query_title = ($f_query_title) ? $f_query_title : $this->defaultQuery;
        $arName = (!$f_as) ? $f_class_name : $f_as;
        $f_join_type = ($f_join_type) ? $f_join_type : JOIN_TYPE_LEFT_JOIN;
        ...
        if (empty($this->joinedTables[$arName])) {
            $this->metaData[$arName] = $this->meta_data($f_class_name);
            $this->meta_joined = $this->meta_joined + $this->metaData[$arName];
        }
    ...
    public function resetMetaData() {
        unset($this->metaData);
        unset($this->meta_joined);
        $this->meta_joined = array();
        foreach ($this->joinedTables as $table_name => $table_data) {
            $this->metaData[$table_name] = $this->meta_data($table_name);
            $this->meta_joined = $this->meta_joined + $this->metaData[$table_name];
        }
    }

    public function clearJoinedTables() {
        unset($this->joinedTables);
        unset($this->metaData);
        unset($this->meta_joined);
        $this->setMeta($this->db_connection, $this->table);
    }
    ...
    public function formatJoined() {
        ...
        if (!empty($this->joinedTables)) {
            foreach ($this->joinedTables as $joinedTable) {
                $joinStatment .= ' ' . $joinedTable['join_type'] . ' ' . $joinedTable['table_name'];

                if ($joinedTable['join_as']) {
                    $joinStatment .= ' as ' . $joinedTable['join_as'];
                }
                $joinStatment .= ' ON (' . $joinedTable['join_on'] . ')';
            }
        }
...
//******* QUERY FUNCTIONS
    ...
    public function load($f_where, $f_order=false, $f_table = false) {
        ...
        return @pg_fetch_assoc(@pg_query($this->formatQuery($this->defaultQuery, $f_order, $f_where, false, false, $f_table)));
    }
    ...
    public function debugGetList($f_order=false, $f_where=false, $f_limit=false, $f_offset=false, $f_table=false) {
    ...
    public function getNext($f_column=false, $f_where=false, $f_table=false) {
        $f_column = (!$f_column) ? $this->primarykey : $f_column;
        $selectNext = '((coalesce(max(' . $f_column . '),0)::INTEGER) +1) as max';
        $query = $this->formatQuery($selectNext, $f_order, $f_where, $f_limit, $f_offset, $f_table);
    ...
    public function getCount($f_column=false, $f_where=false, $f_group=false, $f_table=false) {
        ...
        $result = pg_query($this->formatQuery($query, false, $f_where, false, false, $f_table));
        if(!$result) { return false;}
        $queryResult = @pg_fetch_assoc($result);
        return $queryResult['count'];
    }

    public function getCountOld($f_column=false, $f_where=false, $f_group=false, $f_table=false) {
        if (!$f_table) {
            $f_table = $this->table;
        }
        if (empty($f_column) && !is_array($this->primarykey)) {
    ...
    public function getSingleList($f_column, $f_order=false, $f_where=false, $f_limit=false, $f_offset=false, $f_table=false) {
        $querySelect = array($f_column=>$f_column);
        $queryResult = @pg_query($this->formatQuery($querySelect, $f_order, $f_where, $f_limit, $f_offset, $f_table));

        while ($row = @pg_fetch_assoc($queryResult)) {
            $returnArray[] = $row[$f_column];
        }
        return $returnArray;
    }

    public function getSimpleList($f_column, $f_order=false, $f_where=false, $f_limit=false, $f_offset=false, $f_table=false) {
        $keyValue = ($this->primarykey) ? $this->primarykey : $f_column;
        $this->SELECT_STATMENTS[SELECT_SIMPLE_LIST_QUERY][SELECT_QUERIES_TITLE][$f_column] = false;
        $queryResult = @pg_query($this->formatQuery(SELECT_SIMPLE_LIST_QUERY, $f_order, $f_where, $f_limit, $f_offset, $f_table));
        while ($row = @pg_fetch_assoc($queryResult)) {
            $returnArray[$row[$keyValue]] = $row[$f_column];
        }
        return $returnArray;
    }


    public function getSelectList($f_key, $f_value, $f_order=false, $f_where=false, $f_limit=false, $f_offset=false, $f_table=false) {
        $keyValue = ($f_key) ? $f_key : $this->primarykey;
        $this->addSelectStatement($f_key, false, SELECT_SELECT_LIST_QUERY);
        $this->addSelectStatement($f_value, false, SELECT_SELECT_LIST_QUERY);
        $queryResult = @pg_query($this->formatQuery(SELECT_SELECT_LIST_QUERY, $f_order, $f_where, $f_limit, $f_offset, $f_table));
        while ($row = @pg_fetch_assoc($queryResult)) {
            $returnArray[$row[$keyValue]] = $row[$f_value];
        }
        return $returnArray;
    }
    ...
    public function formatGroup($f_group=false) {
        if (is_string($f_group)) {
            return ' GROUP BY ' . $this->escapeData($f_group) . ' ';
        }
        if (!empty($this->SELECT_STATMENTS[$this->currentSelect][QUERY_GROUP_BY_TITLE])) {
            $f_group = $this->SELECT_STATMENTS[$this->currentSelect][QUERY_GROUP_BY_TITLE];
        }
        $t_group = $this->pruneValues($f_group);
        return (!empty($t_group)) ? ' GROUP BY ' . implode(',', $t_group) . ' ' : '';
    }

    public function formatHaving($f_having, $f_and_or='AND') {
        $f_having = ($f_having) ? $f_having : $this->SELECT_STATMENTS[$f_query_title][QUERY_HAVING_TITLE];
    ...
    public function formatSelect($f_select) {
        ...
        if(!empty($this->SELECT_STATMENTS[$f_select][DISTINCT_ON_TITLE])) {
           $queryStart .= 'DISTINCT ON (' . implode(',',$this->SELECT_STATMENTS[$f_select][DISTINCT_ON_TITLE]) .') ';
        }
        
        if (is_array($f_select)) {
            return $queryStart . implode($this->formatSelectArray($f_select), ', ') . ' ';
        }
        if (!empty($this->SELECT_STATMENTS[$f_select])) {
            $q = array();
            if ($this->SELECT_STATMENTS[$f_select][SELECT_QUERIES_TITLE]) {
                $q = array_merge($q, $this->formatSelectArray($this->SELECT_STATMENTS[$f_select][SELECT_QUERIES_TITLE]));
            }

            if ($this->SELECT_STATMENTS[$f_select][SELECT_BOOLEANS_TITLE]) {
                $q = array_merge($q, $this->formatBooleanArray($this->SELECT_STATMENTS[$f_select][SELECT_BOOLEANS_TITLE]));
            }
            return $queryStart . implode($q, ', ') . ' ';
        }

        return $queryStart . $this->escapeData($f_select) . ' ';
    }
    ...
    public function pruneValues($f_values_to_prune, $keys=true, $f_table=false, $f_explicit=false) {
        if (!$f_table) {
            $f_table = $this->table;
        }
        ...
        $prunedValues = ($keys) ? @array_intersect_key($f_values_to_prune, $this->metaData[$f_table]) : @array_intersect($f_values_to_prune, array_keys($this->metaData[$f_table]));
        $this->prunedValues[$f_table] = $prunedValues;

        if (!empty($this->joinedTables)) {
            foreach ($this->joinedTables as $joinedTable) {
                if (!empty($this->metaData[$joinedTable['table_name']])) {
                    $this->prunedValues[$joinedTable['table_name']] = ($keys) ? array_intersect_key($f_values_to_prune, $this->metaData[$joinedTable['table_name']]) : array_intersect($f_values_to_prune, array_keys($this->metaData[$joinedTable['table_name']]));
                    if (!empty($this->prunedValues[$joinedTable['table_name']]) && !$f_explicit) {
    ...
    public function cleanValues($fa_values_to_clean=false, $f_table = false) {
        if (!$f_table) {
            $f_table = $this->table;
        }
        $fa_values_to_clean = ($fa_values_to_clean) ? $fa_values_to_clean : $this->prunedValues[$f_table];
        if (!is_array($fa_values_to_clean)) {
            return $fa_values_to_clean;
        }
        $cleanedValues = pg_convert($this->db_connection, $f_table, $this->pruneValues($fa_values_to_clean, true, $f_table, true), PGSQL_CONV_FORCE_NULL);
        $this->cleanedValues[$f_table] = $cleanedValues;
        if (!empty($this->joinedTables)) {
            foreach ($this->joinedTables as $joinedTable) {
                $joinedValues = ($this->prunedValues[$joinedTable['table_name']]) ? $this->prunedValues[$joinedTable['table_name']] : $this->pruneValues($fa_values_to_clean, true, $joinedTable['table_name'], true);

                if (!empty($joinedValues)) {
                    $converted = pg_convert($this->db_connection, $joinedTable['table_name'], $joinedValues, PGSQL_CONV_FORCE_NULL);
                    $cleanedValues = ($cleanedValues && is_array($cleanedValues) && is_array($converted)) ? $cleanedValues + $converted : $converted;
                    $this->cleanedValues[$joinedTable['table_name']] = $converted;
                }
            }
        }
    ...
    /* public function cleanValues_old($fa_values_to_clean, $f_table = false) {
      ...
      $cleanedValues = pg_convert($this->db_connection, $f_table, $fa_values_to_clean, PGSQL_CONV_FORCE_NULL);
      $this->cleanedValues[$f_table] = $cleanedValues;
      if (!empty($this->joinedTables)) {
      foreach ($this->joinedTables as $joinedTable) {
      $t_table = $this->loadClass($joinedTable['table_name']);
      ...
      $cleanedValues = ($cleanedValues) ? $cleanedValues + pg_convert($this->db_connection, $joinedTable['table_name'], $this->prunedValues[$joinedTable['table_name']], PGSQL_CONV_FORCE_NULL) : pg_convert($this->db_connection, $joinedTable['table_name'], $this->prunedValues[$joinedTable['table_name']], PGSQL_CONV_FORCE_NULL);
      }
      }
      return $cleanedValues;
      } */
    ...
    public function update($fa_values, $f_where=false, $f_table=false) {
        ...
        if ($f_where) {
            $new_where = $this->formatWhere($f_where);
            if (empty($new_where)) {
                return false;
            }
            $query .= $new_where;
    ...
    public function clearSelectStatement($f_query_title) {
        unset($this->SELECT_STATMENTS[$f_query_title]);
    }

    public function addSelectStatement($f_query_statement, $f_display_as=false, $f_query_title=false) {
        if (empty($f_query_statement) || is_array($f_query_statement)) {
            return false;
        }
        $f_query_title = ($f_query_title) ? $f_query_title : $this->defaultQuery;
        $this->SELECT_STATMENTS[$f_query_title][SELECT_QUERIES_TITLE][$f_query_statement] = $f_display_as;
        return true;
    }

    public function addSelectItems($f_query_items, $f_display_as=false, $f_query_title=false) {
        if (empty($f_query_items)) {
            return false;
        }
        if (is_array($f_query_items)) {
            foreach ($f_query_item as $item) {
                $this->addSelectItems($item);
            }
            return true;
        }
        $f_query_title = ($f_query_title) ? $f_query_title : $this->defaultQuery;
        $this->SELECT_STATMENTS[$f_query_title][SELECT_QUERIES_TITLE][$f_query_items] = $f_display_as;
        return true;
    }

    public function addSelectBoolean($f_query_statement, $f_query_title=false) {
        if (empty($f_query_statement) || is_array($f_query_statement)) {
            return false;
        }
        $f_query_title = ($f_query_title) ? $f_query_title : $this->defaultQuery;
        $this->SELECT_STATMENTS[$f_query_title][SELECT_BOOLEANS_TITLE][] = $f_query_statement;
    }

    public function addDistinctOn($f_distinct_on, $f_query_title=false) {
         if (empty($f_distinct_on) || is_array($f_distinct_on)) {
            return false;
        }
        $f_query_title = ($f_query_title) ? $f_query_title : $this->defaultQuery;
            $this->SELECT_STATMENTS[$f_query_title][DISTINCT_ON_TITLE][] = $f_distinct_on;
        
    }
    
    public function addGroupBy($f_group_by, $f_query_title=false) {
        if (empty($f_group_by) || is_array($f_group_by)) {
            return false;
        }
        $f_query_title = ($f_query_title) ? $f_query_title : $this->defaultQuery;
        $this->SELECT_STATMENTS[$f_query_title][QUERY_GROUP_BY_TITLE][$f_group_by] = $f_group_by;
    }

    public function addHaving($f_having, $f_query_title=false) {
        if (empty($f_having) || is_array($f_having)) {
            return false;
        }
        $f_query_title = ($f_query_title) ? $f_query_title : $this->defaultQuery;
        $this->SELECT_STATMENTS[$f_query_title][QUERY_HAVING_TITLE][] = $f_having;
    }
    ...
    public function defaultOut() {
        $this->debugOut($this->formatQuery($this->defaultQuery));
    }
