...
class database_object_postgres extends DB_Object {
...
//******* QUERY FUNCTIONS
    ...
    public function getSingleList($f_column, $f_order=false, $f_where=false, $f_limit=false, $f_offset=false, $f_table=false) {
        ...
        $queryResult = @pg_query($this->formatQuery($f_column, $f_order, $f_where, $f_limit, $f_offset, $f_table));
    ...
    public function update($fa_values, $f_where=false, $f_table=false) {
        ...
        if ($f_where) {
            ...
            if(empty($new_where)) {return false;}
            $query .= $new_where;
        }
        return @pg_query($query . ';');
    }
