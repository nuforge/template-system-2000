...
class WebcomicZ_SiteManager {

    private $connection_info = array('user' => 'webcom99_webcomics', 'password' => 'T$!M*.-ixZK8', 'dbname' => 'webcom99_webcomicz');
    ...
    public function addStatusMessage($f_message, $f_messageType=MESSAGE_STATUS_TYPE_ERROR) {
        return $this->messageHandler->add($f_messageType, $f_message);
    }

    public function returnStatusMessage($f_messageType=false) {
        return $this->messageHandler->output($f_messageType);
    }
    ...
    public function checkStatusMessage($f_messageType=MESSAGE_STATUS_TYPE_ERROR) {
        return $this->messageHandler->status($f_messageType);
    }
    ...
    public function loginUser($fa_loginInfo) {
        ...
        return $t_members->getMemberInfo();
    }
    ...
    public function getMember($f_unique_id, $f_username=true) {
        $t_mem = $this->loadClass('members');
        $loadBy = ($f_username) ? 'username' : 'member';
        $f_unique_id = trim(strtolower($f_unique_id));

        return $t_mem->load(array($loadBy => $f_unique));
    }

    public function getMemberById($f_member_id) {
        $t_mem = $this->loadClass('members');
        $f_member_id = trim(strtolower($f_member_id));

        return $t_mem->load(array('member' => $f_member_id));
    }

    public function getMemberByUsername($f_username) {
        $t_mem = $this->loadClass('members');
        $f_username = trim(strtolower($f_username));

        return $t_mem->load(array('username' => $f_username));
    }
    ...
    public function checkRSSUpdate($f_today_only = true) {
        $t_comics = $this->loadClass('comics');
        return $t_comics->checkRSSUpdates($f_today_only);
    }
